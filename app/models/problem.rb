class Problem < ActiveRecord::Base
  ...
  def can_buy(profile)
    if self.profiles.include?(profile) or self.auction_mode?() or self.profiles.count >= self.buy_limit or profile.problems.count >= 4
      return false
    end
    # if self.level.price > profile.get_credit
    #   return false
    # end
